...
/**
* AuthLDAPwithWebserver Plugin for LimeSurvey
* Combining plugin AuthLDAP with AuthWebserver 
* based on the Plugin AuthLDAPoverWebserver from Markus Flür <markus.fluer@limesurvey.org>
* @license GPL3.0
*/

class AuthLDAPwithWebserver extends LimeSurvey\PluginManager\AuthPluginBase
{
    protected $storage = 'DbStorage';
    protected static $description = 'Core: LDAP authentication';
    protected static $name = 'AuthLDAPwithWebserver';
    private $oAuthLDAP = null;
    /**
 ...
 /**
    ...
    public function beforeLogin()
    {
        if ($this->get('is_default', null, null, false) == true) {
            // ldap login through webserver authentication
    ...
    public function newUserSession()
    {
        ...
        if ($identity->plugin != 'AuthLDAPwithWebserver') {
            return;
        }
...
//		$password = $this->getPassword(); //not needed for LDAP over Webserver
  ...
  if ($ldapmode == 'searchandbind' && $this->get('autocreate', null, null, false) == true) {
		...
		// pwd not needed for LDAP over Webserver
